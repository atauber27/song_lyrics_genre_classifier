...
model = AutoModelForCausalLM.from_pretrained(
    "fine_tuned_distilgpt"
)
tokenizer = AutoTokenizer.from_pretrained("fine_tuned_distilgpt", padding_side="left")
tokenizer.pad_token = tokenizer.eos_token
f = open("songsForceLong.txt", "a")
i=0
for prompt in prompts:
    ...
    while len(output)<500:
        ...
        generated_ids = model.generate(**model_inputs, max_new_tokens=1024-len(prompt), do_sample=True,repetition_penalty=1.01 ) #max_new_tokens set to not go over max for model
...
# pipe = pipeline(
#     "text-generation",
#     model="fine_tuned_distilgpt",
#     max_new_tokens=1000, #ensures input is not too long
#     repetition_penalty=1.1, #reduce likelyhood of repetition
#     device=0, #use gpu 
#     output_scores=False, #model does not return prompt if this actually does something
#     eos_token_id=None  # Prevent the model from stopping prematurely
# )
